...
import {
  ...
  Typography, IconButton, Box, Chip, Modal, TextField, Button, Grid
} from '@mui/material';
import { teal, red, grey } from '@mui/material/colors';
import EditIcon from '@mui/icons-material/Edit';
import GavelIcon from '@mui/icons-material/Gavel'; // Icône pour représenter une enchère
...
const SellerArticles = () => {
  ...
  const handleAuction = async (articleId) => {
    const confirmation = window.confirm(
      "Êtes-vous sûr de vouloir lancer l'enchère pour cet article ?"
    );
  ...
  return (
    <Box sx={{ backgroundColor: grey[100], minHeight: '100vh', padding: '2rem' }}>
      ...
      <TableContainer component={Paper} sx={{ maxWidth: 900, margin: 'auto', boxShadow: '0 4px 12px rgba(0, 0, 0, 0.1)' }}>
        <Table>
          <TableHead>
  ...
  {articles.map((article) => (
    <TableRow key={article.id}>
      ...
      <TableCell>
        ...
        {article.isApproved && !article.isAuctioned && (
          <Button
            ...
            startIcon={<GavelIcon />} // Ajoute une icône au début du bouton
            sx={{
              bgcolor: teal[700],
              '&:hover': {
                bgcolor: teal[800],
              ...
              borderRadius: 2, // Coins arrondis pour un look plus moderne
              fontWeight: 'bold', // Texte en gras
              textTransform: 'none', // Empêche la transformation du texte en majuscules
              color: 'white',
              padding: '8px 16px', // Espace supplémentaire pour plus de confort
            }}
          >
            Lancer l'enchère
      </Button> )}
      ...
      {/* Modale pour modifier l'article avec les images */}
